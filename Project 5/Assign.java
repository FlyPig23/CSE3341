...
class Assign {
    ...
    void execute() {
        if (declareType == 1) {
            ...
            GarbageCollector.allocateToGarbageCollector();
            System.out.println("gc:" + GarbageCollector.countTotalReference());
        } else if (declareType == 3) {
            if (Executor.searchCorrespondingCoreVar(iLeft.stringForm()).value != null) {
                iLeft.referenceAssign(iRight);
                GarbageCollector.subFromGarbageCollector(iLeft.stringForm());
                GarbageCollector.addToGarbageCollector(iLeft.stringForm());
                GarbageCollector.checkReachZero(iLeft.stringForm());
            } else {
                iLeft.referenceAssign(iRight);
                GarbageCollector.addToGarbageCollector(iLeft.stringForm());
                GarbageCollector.checkReachZero(iLeft.stringForm());
